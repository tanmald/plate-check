...
export default function MealResult() {
  ...
  const getConfidenceLabel = (confidence: string) => {
    switch (confidence) {
      case "high":
        return { label: t("mealResult.confidence_high"), color: "text-success" };
      case "medium":
        return { label: t("mealResult.confidence_medium"), color: "text-warning" };
      default:
        return { label: t("mealResult.confidence_low"), color: "text-muted-foreground" };
    }
  };
  ...
  if (isFetchingMeal) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <p className="text-sm text-muted-foreground">{t("mealResult.loading_meal")}</p>
      </div>
    );
  }
